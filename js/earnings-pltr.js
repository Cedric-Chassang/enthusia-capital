// Palantir Q1 2026 income-statement Sankey, scoped for embedding inside a
// dynamic container on the company page. Renders into #earnings-slot.
// NOTE: GAAP net income ($871M) > Op. income ($754M) due to $134M non-operating
// income; Sankey terminates at op income, net income/EPS are in the KPI strip.
(function () {
  ...
  const NODES = [
    { id: "us_commercial", label: "U.S. Commercial",   value: 595,  prev_q: 504,  prev_y: 255, col: 0, color: "#3b82f6" },
    { id: "us_government", label: "U.S. Government",   value: 687,  prev_q: 568,  prev_y: 373, col: 0, color: "#3b82f6" },
    { id: "international", label: "International",     value: 351,  prev_q: 336,  prev_y: 256, col: 0, color: "#60a5fa" },
    { id: "total_rev",     label: "Total Revenue",     value: 1633, prev_q: 1408, prev_y: 884, col: 1, color: "#2563a8" },
    { id: "gross_profit",  label: "Gross Profit",      value: 1417, prev_q: null, prev_y: 711, col: 2, color: "#16a34a" },
    { id: "cost_rev",      label: "Cost of Revenue",   value: 216,  prev_q: null, prev_y: 173, col: 2, color: "#dc2626" },
    { id: "op_income",     label: "Op. Income",        value: 754,  prev_q: null, prev_y: 176, col: 3, color: "#15803d" },
    { id: "op_expenses",   label: "Op. Expenses",      value: 663,  prev_q: null, prev_y: 535, col: 3, color: "#b91c1c" },
    { id: "sales_mktg",    label: "Sales & Marketing", value: 319,  prev_q: null, prev_y: 236, col: 4, color: "#b91c1c" },
    { id: "rd",            label: "R&D",               value: 161,  prev_q: null, prev_y: 135, col: 4, color: "#b91c1c" },
    { id: "ga",            label: "G&A",               value: 183,  prev_q: null, prev_y: 164, col: 4, color: "#b91c1c" },
  ];

  const LINKS = [
    { s: "us_commercial", t: "total_rev",    v: 595 },
    { s: "us_government", t: "total_rev",    v: 687 },
    { s: "international", t: "total_rev",    v: 351 },
    { s: "total_rev",     t: "gross_profit", v: 1417 },
    { s: "total_rev",     t: "cost_rev",     v: 216 },
    { s: "gross_profit",  t: "op_income",    v: 754 },
    { s: "gross_profit",  t: "op_expenses",  v: 663 },
    { s: "op_expenses",   t: "sales_mktg",   v: 319 },
    { s: "op_expenses",   t: "rd",           v: 161 },
    { s: "op_expenses",   t: "ga",           v: 183 },
  ];
  ...
  const KPI = [
    { label: "Revenue",      actual: "$1.63B", est: "$1.54B", beat: true },
    { label: "Gross Margin", actual: "86.8%",  est: null,     beat: null },
    { label: "Adj. EPS",     actual: "$0.33",  est: "$0.28",  beat: true },
  ];
  const GUIDANCE = { value: "$1.80B", note: "vs. Q2 est. $1.68B (LSEG)" };
  ...
  function buildSVG() {
    ...
    return `<svg width="${W}" height="${H}" viewBox="0 0 ${W} ${H}" xmlns="http://www.w3.org/2000/svg" style="border:1px solid #e2e8f0;border-radius:6px;display:block;max-width:100%;height:auto">
      ...
      <text x="${W - 12}" y="${legY}" text-anchor="end" font-size="9" fill="#94a3b8" font-family="system-ui,sans-serif">Source: Palantir Q1 2026 Earnings · investors.palantir.com</text>
  ...
  function downloadPNG() {
    ...
    img.onload = () => {
      ...
      a.download = 'palantir_q1_2026.png'; a.href = canvas.toDataURL('image/png'); a.click();
    };
    img.src = url;
  }
  window.__downloadEarningsPNG_PLTR = downloadPNG;

  target.innerHTML = `
    <div class="earnings-chart-wrap">
      ${buildSVG()}
      <button class="dl-png-btn" onclick="window.__downloadEarningsPNG_PLTR()">
  ...
  window.__enthusiaEarnings['PLTR'] = { ticker: 'PLTR', period: 'Q1 2026', KPI, GUIDANCE, download: downloadPNG };
  window.dispatchEvent(new CustomEvent('enthusia-earnings-ready', { detail: { ticker: 'PLTR' } }));
})();
